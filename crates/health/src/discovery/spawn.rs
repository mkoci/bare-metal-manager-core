...
use crate::collectors::{
    ...
    SensorCollector, SensorCollectorConfig, create_log_file_writer, spawn_gnmi_collector,
};
        ...
        match Collector::start::<NvueRestCollector>(
            ...
            Err(error) => {
                tracing::error!(
                    ?error,
                    endpoint_key = %key,
                    "Could not start NVUE REST collector for switch"
                );
            }
        }
    }
        ...
        match spawn_gnmi_collector(endpoint, gnmi_cfg, collector_registry, data_sink.clone()) {
            Ok(handle) => {
                ...
                tracing::info!(
                    ...
                    "Started NVUE gNMI collection for switch endpoint"
                );
            }
            Err(error) => {
                tracing::error!(
